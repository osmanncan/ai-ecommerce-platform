...
export default function AdminOrders() {
    const [orders, setOrders] = useState([
        { id: "#ORD-9128", customer: "Berke YÄ±lmaz", email: "berke@example.com", items: 3, total: 11250, date: "12 Mar 2026, 14:30", status: "KargolandÄ±" },
        { id: "#ORD-9127", customer: "AslÄ± Ã‡elik", email: "asli@example.com", items: 1, total: 8900, date: "12 Mar 2026, 09:15", status: "HazÄ±rlanÄ±yor" },
        { id: "#ORD-9126", customer: "Kaan Aras", email: "kaan@example.com", items: 4, total: 4200, date: "11 Mar 2026, 18:45", status: "Beklemede" },
        { id: "#ORD-9125", customer: "Zeynep Demir", email: "zeynep@example.com", items: 2, total: 18500, date: "10 Mar 2026, 11:20", status: "TamamlandÄ±" },
        { id: "#ORD-9124", customer: "Can Kaya", email: "can.k@example.com", items: 1, total: 2450, date: "09 Mar 2026, 16:00", status: "Ä°ptal Edildi" },
    ]);

    const getStatusStyle = (status: string) => {
        switch (status) {
            case "TamamlandÄ±": return "bg-emerald-50 text-emerald-600";
            case "KargolandÄ±": return "bg-blue-50 text-blue-600";
            case "HazÄ±rlanÄ±yor": return "bg-indigo-50 text-indigo-600";
            case "Beklemede": return "bg-orange-50 text-orange-600";
            case "Ä°ptal Edildi": return "bg-red-50 text-red-600";
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            case "TamamlandÄ±": return <CheckCircle size={14} />;
            case "KargolandÄ±": return <Plane size={14} />;
            case "HazÄ±rlanÄ±yor": return <Clock size={14} />;
            case "Beklemede": return <Clock size={14} className="animate-spin" />;
            case "Ä°ptal Edildi": return <XCircle size={14} />;
            default: return null;
        }
    };
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black tracking-tighter uppercase text-zinc-900 leading-none">SipariÅŸler</h1>
                    <p className="text-zinc-400 text-sm mt-2 font-medium">Toplam {orders.length} sipariÅŸ listeleniyor.</p>
                </div>
                <div className="flex gap-2">
                    <button className="px-6 py-3 bg-white border border-zinc-200 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-zinc-50 transition-colors">DÄ±ÅŸa Aktar</button>
                </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-4 bg-white p-4 rounded-2xl border border-zinc-200">
                <div className="flex-1 relative">
                    ...
                    <input
                        type="text"
                        placeholder="MÃ¼ÅŸteri adÄ±, e-posta veya sipariÅŸ no..."
            ...
            <div className="bg-white rounded-3xl border border-zinc-200 overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-zinc-50 border-b border-zinc-200">
                            <tr>
                                <th className="text-left py-5 px-8 text-[10px] font-black uppercase tracking-widest text-zinc-400">SipariÅŸ ID</th>
                                <th className="text-left py-5 px-8 text-[10px] font-black uppercase tracking-widest text-zinc-400">MÃ¼ÅŸteri DetayÄ±</th>
                                <th className="text-left py-5 px-8 text-[10px] font-black uppercase tracking-widest text-zinc-400">Tarih</th>
                                <th className="text-left py-5 px-8 text-[10px] font-black uppercase tracking-widest text-zinc-400">Tutar / Ä°Ã§erik</th>
                                <th className="text-left py-5 px-8 text-[10px] font-black uppercase tracking-widest text-zinc-400">Durum</th>
                                <th className="py-5 px-8 text-right text-[10px] font-black uppercase tracking-widest text-zinc-400">Ä°ÅŸlemler</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={order.id} className="hover:bg-zinc-50/50 transition-colors group">
                                    ...
                                    <td className="py-6 px-8">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mt-1">{order.items} ÃœrÃ¼n</span>
                                        </div>
                                    </td>
                                    <td className="py-6 px-8">
                ...
                <div className="p-4 border-t border-zinc-200 flex items-center justify-between text-xs font-bold text-zinc-500 bg-zinc-50/50">
                    <span>GÃ¶sterilen: 1-5 / Toplam: 124</span>
                    <div className="flex gap-1">
                        <button className="px-3 py-1.5 border border-zinc-200 rounded bg-white hover:bg-zinc-50 disabled:opacity-50" disabled>Ã–nceki</button>
